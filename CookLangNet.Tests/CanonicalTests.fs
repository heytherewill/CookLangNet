...
module TestFailedDueTo =
    ...
    %s"""
                canonicalTest.TestSuiteVersion
                canonicalTest.Name
                errorMessage
        ...
        let steps =
            let toStep (rawItem: obj) =
                ...
                for item in itemList do
                    ...
                    | "text" -> directions <- directions + (itemDictionary["value"].ToString())
                    | "ingredient" ->
                        ...
                        let quantity =
                            ...
                            if quantityString = "some" then
                                None
                            else
                                quantityFromString quantityString |> Some
                            ...
                            { Name = name
                              Amount =
                                ...
                                | Some quantity ->
                                    Some
                                        { Quantity = quantity
                                          Unit = if units = "" then None else Some units } }
            ...
            yamlDictionary["steps"] :?> List<obj> |> Seq.map toStep |> List.ofSeq
