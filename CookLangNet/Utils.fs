...
let quantityFromString (quantity: string) =
    ...
    let parsedFraction =
        ...
        if (symbolLocation < 0) then
            None
        else
            let numerator = quantity.Substring(0, symbolLocation)

            let denominator =
                quantity.Substring(symbolLocation + 1, quantity.Length - symbolLocation - 1)

            if not (Double.TryParse(numerator, numberRef)) then
                None
            else
                let parsedNumerator = numberRef.Value

                if not (Double.TryParse(denominator, numberRef)) then
                    None
                else
                    let parsedDenominator = numberRef.Value

                    if (parsedDenominator > 0) then
                        Numeric(parsedNumerator / parsedDenominator) |> Some
                    else
                        None
    ...
    | Some q -> q
    | None ->
        if Double.TryParse(quantity, numberRef) then
            Numeric numberRef.Value
        else
            Textual(trim quantity)
